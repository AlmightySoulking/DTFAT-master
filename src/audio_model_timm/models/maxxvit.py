...
from audio_model_timm.data import IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD
from audio_model_timm.layers import Mlp, ConvMlp, DropPath, LayerNorm, ClassifierHead, NormMlpClassifierHead
from audio_model_timm.layers import create_attn, get_act_layer, get_norm_layer, get_norm_act_layer, create_conv2d, create_pool2d
from audio_model_timm.layers import trunc_normal_tf_, to_2tuple, extend_tuple, make_divisible, _assert
from audio_model_timm.layers import RelPosMlp, RelPosBias, RelPosBiasTf, use_fused_attn
...
class MbConvBlock_TF(nn.Module):
    ...
    def __init__(
            ...
            window_size_freq = None,
    ):
        super(MbConvBlock_TF, self).__init__()
        ...
        if stride_2==1:
            self.conv2_kxk_t = nn.Conv2d(mid_chs, mid_chs, kernel_size=window_size_time, stride=stride_2, padding='same', groups=mid_chs, bias=False)
            self.conv2_kxk_f = nn.Conv2d(mid_chs, mid_chs, kernel_size=window_size_freq, stride=stride_2, padding='same', groups=mid_chs, bias=False)
            print("window_size_time,window_size_freq ",window_size_time,window_size_freq)
            self.t_f_weight = torch.nn.Parameter(torch.tensor(0.5), requires_grad=True)

            # print("stride 1 same pad ",count_parameters(self.conv2_kxk_t)/1e6)


        elif stride_2==2:
            self.conv2_kxk_t = Conv2dSame(in_channels=mid_chs,out_channels=mid_chs,kernel_size=window_size_time,stride=stride_2,groups=mid_chs, bias=False)
            self.conv2_kxk_f = Conv2dSame(in_channels=mid_chs,out_channels=mid_chs,kernel_size=window_size_freq,stride=stride_2,groups=mid_chs, bias=False)
            print("In stride 2 window_size_time,window_size_freq ",window_size_time,window_size_freq)

            self.t_f_weight = torch.nn.Parameter(torch.tensor(0.5), requires_grad=True)
            # print("stride 2 same pad ",count_parameters(self.conv2_kxk_t)/1e6)

            # self.conv2_kxk_t = None
        else:
            exit('stride unknow')
    ...
    def forward(self, x):
        ...
        ## decouple 
        if self.conv2_kxk_t is not None:
            x_t = self.conv2_kxk_t(x)
            x_f = self.conv2_kxk_f(x)
            # x_1 = self.conv2_kxk(x)        

            x = (self.t_f_weight*x_t) + ((1-self.t_f_weight)*x_f) #+ x_1
        else:
            print('\n warning no cases with conv_t none')
            # x=self.conv2_kxk(x) 
            exit("error self.conv2_kxk_t")
...
class PartitionAttentionCl(nn.Module):
    ...
    def __init__(
        ...
        # print(cfg.window_size,cfg.grid_size)
        # exit('fg')
        # self.partition_size = to_2tuple(cfg.window_size if self.partition_block else cfg.grid_size)
        self.partition_size = window_size_new

        # print("self.partition_size", self.partition_size)
        # self.partition_size = window_size_new
...
class ParallelPartitionAttention_TF(nn.Module):
    """ Experimental. Grid and Block partition + single FFN
    NxC tensor layout.

    Grid and Block in parallel then concat, see maxvit paper Sequential Vs Parallel
    """
    ...
    def __init__(
        ...
        # assert cfg.window_size == cfg.grid_size(16)
        # self.partition_size = to_2tuple(cfg.window_size)
        self._partition_size_t = window_size_tf[0]
        self._partition_size_f = window_size_tf[1]
        
        rel_pos_cls_t = get_rel_pos_cls(cfg, self._partition_size_t)
        rel_pos_cls_f = get_rel_pos_cls(cfg, self._partition_size_f)



        self.norm1 = norm_layer(dim)
        self.attn_block_t = AttentionCl(
            dim,
            dim,
            dim_head=cfg.dim_head,
            bias=cfg.attn_bias,
            head_first=cfg.head_first,
            rel_pos_cls=rel_pos_cls_t,
            attn_drop=cfg.attn_drop,
            proj_drop=cfg.proj_drop,
        )
        self.attn_block_f = AttentionCl(
            dim,
            dim,
            dim_head=cfg.dim_head,
            bias=cfg.attn_bias,
            head_first=cfg.head_first,
            rel_pos_cls=rel_pos_cls_f,
            attn_drop=cfg.attn_drop,
            proj_drop=cfg.proj_drop,
        )
        self.ls1 = LayerScale(dim, init_values=cfg.init_values) if cfg.init_values else nn.Identity()
    ...
    def _partition_attn(self, x):
        ...
        partitioned_block = window_partition(x, self._partition_size_t)
        partitioned_block = self.attn_block_t(partitioned_block)
        x_t = window_reverse(partitioned_block, self._partition_size_t, img_size)

        partitioned_block = window_partition(x, self._partition_size_f)
        partitioned_block = self.attn_block_f(partitioned_block)
        x_f = window_reverse(partitioned_block, self._partition_size_f, img_size)

        # print("x_t ",x_t.shape)

        return x_t + x_f
        # return torch.cat([x_t,x_f], dim=-1)
...
class ParallelPartitionAttention(nn.Module):
    ...
    def forward(self, x):
        exit('in ParallelPartitionAttention')
        ## not using
        x = x + self.drop_path1(self.ls1(self._partition_attn(self.norm1(x))))
        x = x + self.drop_path2(self.ls2(self.mlp(self.norm2(x))))
        return x
...
class MaxxVitBlock(nn.Module):
    ...
    def __init__(
        ...
        # conv_cls = ConvNeXtBlock if conv_cfg.block_type == 'convnext' else MbConvBlock_TF
        self.conv = MbConvBlock_TF(dim, dim_out, stride=stride, cfg=conv_cfg, drop_path=drop_path,input_size=input_size,window_size_time=window_size_time,window_size_freq=window_size_freq)
        # print("count_parameters ",count_parameters(self.conv))

        attn_kwargs = dict(dim=dim_out, cfg=transformer_cfg, drop_path=drop_path, window_size_new=window_size_new)
        ...
        for i, t in enumerate(block_types):
            ...
            if t == 'C':
                ...
                blocks += [MaxxVitBlock(
                    ...
                    drop_path=drop_path[i],
                    window_size_new=window_size_new,
...
class Stem(nn.Module):
    ...
    def __init__(
        ...
        self.conv2_t = torch.nn.Conv2d(in_channels=out_chs[0], out_channels=out_chs[0], kernel_size=t_kernel, padding='same', stride=1, groups=1, bias=bias)
        self.conv2_f = torch.nn.Conv2d(in_channels=out_chs[0], out_channels=out_chs[0], kernel_size=f_kernel, padding='same', stride=1, groups=1, bias=bias)

        print("in stem stride 1 same pad ",count_parameters(self.conv1_f))
        print("in stem stride 2 same pad ",count_parameters(self.conv2_f))
...
class MaxxVit(nn.Module):
    ...
    def __init__(
        ...
        self.stem = Stem(
            ...
            t_f_kernel=window_size_time_freq[0],
        )
        ...
        for i in range(num_stages):
            print("window_size ",window_size_list[i])
            stage_stride = 2
            ...
            stages += [MaxxVitStage(
                ...
                feat_size=feat_size,
                drop_path=dpr[i],
                window_size_new=window_size_list[i],
                window_size_time=window_size_time_freq[i][0],
                window_size_freq=window_size_time_freq[i][1],
                input_size=feat_map_size_list[i]
    ...
    def forward_features(self, x):
        ...
        # x = self.stages(x)
        for n,l in enumerate(self.stages):
            # print("count_parameters ",count_parameters(l)/1e6)
            x = l(x)
    
        x = self.norm(x)
        # print("self.norm out ",x.shape)
...
def _cfg(url='', **kwargs):
    return {
        'url': url, 'num_classes': 2, 'input_size': (3, 224, 224), 'pool_size': (7, 7),
